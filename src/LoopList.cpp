...
void LoopList::FindChildren(){
	...
	while (i < j){  // check all bases i from starting base pair of multiloop to ending base pair
		if (Input->ClosedRegions[i] != NULL){  // if base i starts a closed region
			Input->loops[base1].bri[Input->loops[base1].num_branches++] = i;  // set 'i'th branch to start of 'i'th closed region in multiloop
			...
			if (DEBUG)
				printf("$$$$$$$$$$$$$$$$[%d, %d] is a child of (%d, %d)\n", Input->ClosedRegions[i]->begin, Input->ClosedRegions[i]->end
									, base1, base2);
			i = Input->Next[Input->ClosedRegions[i]->end];  // go to start of next closed region (next base pair after end of current closed region)
		}
		else
...
//	misc_energy += misc.multi_helix_penalty * f[i].pseudo_num_branches  + 2 * p_pairedMultiPseudo;
	...
	if (DEBUG2)
		printf("[multiPseudoEnergy DP]: f[i].num_branches + 2 = %d, f[i].pseudo_num_branches = %d\n", f[i].num_branches + 2, f[i].pseudo_num_branches);
