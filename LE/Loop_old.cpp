...
void Loop::LoopFindBands(){
	...
	if (DEBUG) {
		printf("[updating links](%d, %d) ==> (%d, %d)\n", begin, end, bandpattern->Prev(begin), bandpattern->Next(end)); 
		fflush(stdout);
	}
...
/*
	switch (type){
		case	stackloop:
			printf("+++ WARNING: Incorrectly detected a stackloop -- should be multiloop since has a pk branch\n");
...
//			printf("***[%d, %d] Stacked Pair Energy = %f cal/mol\n", begin, end, 10*energyToAdd);
			...
			break;
		case	hairpin	:
			printf("+++ WARNING: Incorrectly detected a hairpin -- should be multiloop since has a pk branch\n");
...
//			printf("***[%d, %d] Internal Loop Energy = %f cal/mol\n", begin, end, 10*energyToAdd);
			...
			break;
		case	multi	:
			energyToAdd = multiEnergyDP(c, f, reset_c, ignore_dangles);
//			energyTrace += "***[" + begin + ", " + end + "] Multiloop Energy = " + 10*energyToAdd + " cal/mol\n";
//			printf("***[%d, %d] Multiloop Energy = %f cal/mol\n", begin, end, 10*energyToAdd);
			sum += energyToAdd;
			break;
		case	pseudo	:
			energyToAdd = pseudoEnergyDP(c, f, reset_c, ignore_dangles);
//			energyTrace += "***[" + begin + ", " + end + "] Pseudoloop Energy = " + 10*energyToAdd + " cal/mol\n";
//			printf("***[%d, %d] Pseudoloop Energy = %f cal/mol\n", begin, end, 10*energyToAdd);
			sum += energyToAdd;
			break;
						...
						&& Input->looplists[L1->Num]->base1 <= current_border)
					{
						...
						if (ap == a+1)  // stack spanning a band
						{
							...
							for (int i = 0; i < num_params_pkfree; i++)
							{
								...
								P_matrix[i][num_params_pkfree + structure_type_index_PK("stp") - 1] += c_temp[i];
							}
						...
						else  // internal loop spanning band
						{
							...
							{
								// if c_temp[i] == 0, P_matrix remains unchanged, as needed
								P_matrix[i][num_params_pkfree + structure_type_index_PK("intp") - 1] += c_temp[i];
							}
							// update the single stP entry in counter
...
//				+ pkmodelDP.Pup * NumberOfUnpaird );  // number of unpaired bases
	...
	if (DEBUG2)
		printf("[PseudoEnergy] wit energy of loops spanning bands (10cal/mol): %f, NumBands: %d, NumUnpaired: %d, NumberOfUnpairedInPseudo = %d, NumOfBandChild = %d, NumberOfUnBandChild: %d\n", 
				100*Energy, NumberOfBands, NumberOfUnpaird, NumberOfUnpairedInPseudo, NumberOfBandChild, NumberOfUnBandChild);
...
float	Loop::pseudoEnergyCC2006b(){
	...
	for (k = border_bp; k < border_b; k++)
	{
		if (Input->BasePair(k) != validBasepair)
		{
			if (Input->BasePair(k) > border_bp) // if base pair is less than c' above
			{
				multi = 1;
				break;  // break since not handled by Cao & Chen
			}
	...
	for (k = border_dp; k < border_d; k++)
	{
		if (Input->BasePair(k) != validBasepair)
		{
			if (Input->BasePair(k) > border_dp) // if base pair is less than c' above
			{
				multi = 1;
				break;  // break since not handled by Cao & Chen
			}
